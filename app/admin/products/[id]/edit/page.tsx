...
export default function ProductEdit() {
  ...
  const handleAddTag = (setFieldValue: any) => {
    if (tagInput.trim() && !tags.includes(tagInput.trim())) {
      ...
      setTagInput('');
      toast({ title: 'Tag Added', description: `Tag "${tagInput.trim()}" has been added.` });
    } else if (tags.includes(tagInput.trim())) {
      toast({ title: 'Duplicate Tag', description: 'This tag already exists.', variant: 'error' });
    }
  };
  ...
  const handleAddProductSize = (setFieldValue: any) => {
    if (productSizeInput.trim() && !productSize.includes(productSizeInput.trim())) {
      ...
      setProductSizeInput('');
      toast({ title: 'Product Size Added', description: `Product size "${productSizeInput.trim()}" has been added.` });
    } else if (productSize.includes(productSizeInput.trim())) {
      toast({ title: 'Duplicate Product Size', description: 'This product size already exists.', variant: 'error' });
    }
  };
  ...
  const handleRemoveProductSize = (index: number, setFieldValue: any) => {
    ...
    setProductSize(newProductSize);
    setFieldValue('productSize', newProductSize);
    toast({ title: 'Product Size Removed', description: 'Product size has been removed.' });
  };
  ...
  const handleAddKeyword = (setFieldValue: any) => {
    if (keywordInput.trim() && !seoKeywords.includes(keywordInput.trim())) {
      ...
      setKeywordInput('');
      toast({ title: 'Keyword Added', description: `SEO keyword "${keywordInput.trim()}" has been added.` });
    } else if (seoKeywords.includes(keywordInput.trim())) {
      toast({ title: 'Duplicate Keyword', description: 'This SEO keyword already exists.', variant: 'error' });
    }
  };
  ...
  const handleAddVideoLink = (setFieldValue: any) => {
    ...
    if (!trimmedUrl) {
      toast({ title: 'Empty URL', description: 'Please enter a video URL.', variant: "error", });
      return;
    }

    if (!isValidVideoUrl(trimmedUrl)) {
      toast({ title: 'Invalid URL', description: 'Please enter a valid YouTube, Vimeo, or direct video URL.', variant: "error", });
      return;
    }

    if (videoLinks.includes(trimmedUrl)) {
      toast({ title: 'Duplicate Video', description: 'This video link already exists.', variant: "error", });
      return;
    }
    ...
    setVideoInput('');
    toast({ title: 'Video Added', description: `Video ${newVideoLinks.length} has been added successfully.` });
  };
  ...
  const handleRemoveVideoLink = (index: number, setFieldValue: any) => {
    ...
    setVideoLinks(newVideoLinks);
    setFieldValue('videoLinks', newVideoLinks);
    toast({ title: 'Video Removed', description: 'Video has been removed.' });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button variant="outline" size="sm" onClick={() => router.push(`/admin/products/${params.id}`)}>
              <ArrowLeft size={16} className="mr-2" />
              Back
            </Button>
            <div>
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            try {
              ...
              if (response.ok) {
                ...
                toast({ title: 'Error', description: errorData.message || 'Failed to update product', variant: "error", });
              }
            } catch (error) {
        ...
        >
          {({ values, setFieldValue, isSubmitting, errors, touched, dirty }) => {
            // Initialize state from form values if they're empty
            React.useEffect(() => {
                ...
                <div className="space-y-6">
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <Button type="submit" disabled={saving || isSubmitting || !dirty} className="w-full">
